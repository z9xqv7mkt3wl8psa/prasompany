...
interface Result {
    status?: string;
    error?: string;
    certificate?: CertificateVerifyData;
}
            ...
            const verifyToken = async () => {
                try {
                    ...
                    if (!querySnapshot.empty) {
                        ...
                        const assignedFullProjectName = certificateDoc.data()?.assignedFullProjectName || 'N/A';

                        const certificateData: CertificateVerifyData = {
                            ...docData as CertificateVerifyData,
                            assignedFullProjectName: assignedFullProjectName,
                        };
                ...
                {result?.status === 'Verified' && result.certificate && (
                    <div style={{ textAlign: 'center', padding: '20px', border: '1px solid green', backgroundColor: '#e6ffe6' }}>
                        ...
                        <div style={{ textAlign: 'left', marginTop: '20px' }}>
                            ...
                            <p><strong>Internship Type:</strong> {result.certificate.internshipType}</p>
                            <p><strong>Internship Duration:</strong> {result.certificate.internshipDuration}</p>
                            <p><strong>Assigned Project:</strong> {result.certificate.assignedFullProjectName}</p>
                        </div>
                    </div>
                )}
            </div>
